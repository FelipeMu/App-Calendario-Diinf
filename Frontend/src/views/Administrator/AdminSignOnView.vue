  ...
  <div class="global-container">
    <div class="signup">
      ...
      <div class="right-box">
        ...
        <h4 STYLE="color:#000">ADMINISTRADOR</h4>
        <form action="/admin-calendar">
          <label class="input-label" for="email-user-admin">Correo electrónico</label>
          <input type="email" placeholder="Ingrese su correo" required class="input-box" id="email-user-admin">
          <label class="input-label" for="password-user-admin">Contraseña</label>
          <input type="password" placeholder="Ingrese su contraseña" required class="input-box" id="password-user-admin">
          <button class="btn-signup" type="submit" @click.prevent="validacionTotal()">Ingresar</button>  
        </form>
        <a href="/">Iniciar sesión como profesor</a>
      </div>
    </div>
  </div>
  <footer>
...
export default {
  ...
  data: () => ({ 
    ...
    datos_admin: null,//json recibido por api rest
    activador: false,
  ...
  methods: {
    ...
    AnalizarContrasena(){
      let contrasena_ingresada = document.getElementById('password-user-admin').value
      if(contrasena_ingresada == this.datos_admin.contrasena){
    ...
    async ExisteUsuario(){
      ...
      let correo_ingresado = document.getElementById("email-user-admin").value; 
      // método get - axios
      await axios
      .get("/datosadmin/" + correo_ingresado)
      .then((result) => {
        this.datos_admin = result.data;
        this.id_admin = this.datos_admin.id;
        this.nombre_admin = this.datos_admin.nombre;
      }).catch(e => {
            console.log(e);
      ...
      if(this.$options.methods.validarCorreo.bind(this)() && this.$options.methods.validarContrasena.bind(this)()){
        ...
        if(this.activador && this.activador_pass){
          ...
          if(this.activador && this.activador_pass==false){
            ...
            console.log(this.activador_pass);
            Swal.fire({
            icon: 'error',
            title: 'Oops...',
    ...
    */
    validarCorreo(){   
      var emailField = document.getElementById('email-user-admin');     
    ...
    validarContrasena(){
      var passField = document.getElementById('password-user-admin');     
      // Expresion regular para validar el correo electronico institucional
